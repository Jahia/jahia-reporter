...
const buildTest = (xmlTests: any) => xmlTests.map((t: any) => {
  ...
  return {
    ...t.attributes,
    failures: t.elements === undefined ? [] : t.elements.map((f: any) => JSON.stringify(f.elements)),
    status,
...
const buildSuites = (xmlSuites: any, testFilename: string) => xmlSuites.map((s: any) => ({
  ...s.attributes,
