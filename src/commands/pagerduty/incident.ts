  ...
  async run() {
    ...
    if (flags.googleSpreadsheetId === '') {
      ...
      for (const row of spRows) {
        if (row.get('Test Service') === flags.service) {
          if (
            row.get('PagerDuty Enabled') !== undefined &&
            row.get('PagerDuty Enabled').toLowerCase() === 'no'
          ) {
            pagerDutyNotifEnabled = false;
          }

          if (
            row.get('PagerDuty Service ID') !== undefined &&
            row.get('PagerDuty Service ID').length > 4
          ) {
            pagerDutyServiceId = row.get('PagerDuty Service ID');
          }

          if (row.get('PagerDuty User ID') !== undefined) {
            for (const assignee of row
              .get('PagerDuty User ID')
              .split(',')
              .filter((a: string) => a.length > 4)) {
          ...
          if (
            ...
            (row.get('CI/CD') === 'Bamboo' || row.get('CI/CD') === 'GitHub')
          ) {
            this.log(`Updated state for: ${flags.service}`);
            row.set('State', testFailures > 0 ? 'FAILED' : 'PASSED');
            row.set('Updated', new Date().toISOString());
            row.set('Total', testTotal);
            row.set('Failures', testFailures);
            this.log(
