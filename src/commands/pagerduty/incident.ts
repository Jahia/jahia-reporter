...
const getSpreadsheetRows = async (
  ...
  // Note: GoogleSpreadsheet v5 API may have changed - this might need updating
  ...
  } as any); // Temporary any cast for API compatibility
  await doc.loadInfo();
...
class JahiaPagerDutyIncident extends Command {
  ...
  async run() {
    const { flags } = await this.parse(JahiaPagerDutyIncident);
...
export default JahiaPagerDutyIncident;
